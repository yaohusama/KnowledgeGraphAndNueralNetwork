...
class Bi_LSTM_Task:
    ...
    def add_placeholders(self):
        self.word_ids = tf.placeholder(tf.int32, shape=[self.batch_size, 9], name="word_ids")#7
    ...
    def lookup_layer_op(self):
        ...
        with tf.variable_scope("words"):
            ...
            self.word_embeddings_key = tf.nn.embedding_lookup(params=_word_embeddings_key,
                                                              ids=self.word_ids,
                                                              name="word_embeddings")
            # self.embeddings_key = tf.one_hot(self.word_ids, depth=self.key_class)
            # w_task_kw = tf.get_variable(name="w_task_kw",  # w_task_kw与encoder_final_state_h进行矩阵乘法，得到对类别的映射
            #                             shape=[self.key_class, self.embedding_dim],  # intent的个数
            ...
            #                             dtype=tf.float32)
            # # task
            # self.word_embeddings_key = tf.add(tf.matmul(self.embeddings_key, w_task_kw), b_task_kw)  # 异构网络1
    ...
    def biLSTM_layer_op(self):
        with tf.variable_scope("bi-lstm"):
            ...
            print("attention_feature",att_fea)


        with tf.variable_scope("proj"):
            merged_f=tf.add(att_fea,merged_feature)
            new_f=tf.reshape((merged_f)[0],[1,-1])
            tmp=tf.reshape((merged_f)[0],[1,-1])
            for i in range(1,merged_f.shape[0]):
                x=tf.add(tf.reshape(merged_f[i],[1,-1]),tmp)
                x=tf.divide(x,i+1)
                new_f = tf.concat((new_f, x), axis=0)
                tmp=x
            self.feature=new_f
            w_task_kw = tf.get_variable(name="w_task_kw",  # w_task_kw与encoder_final_state_h进行矩阵乘法，得到对类别的映射
                                        shape=[2 * self.hidden_dim, self.au_class],  # intent的个数
                                        initializer=tf.contrib.layers.xavier_initializer(),
                                        dtype=tf.float32)
            b_task_kw = tf.get_variable(name="b_task_kw",
                                        shape=[self.au_class],
                                        ...
                                        dtype=tf.float32)
            # task
            logits = tf.add(tf.matmul(new_f, w_task_kw), b_task_kw)  # 异构网络1
            print('logits:', logits)
    ...
    def pad_sequences(self, sequences, pad_mark=0, predict=False, vocab_flag=0):
        ...
        for seq in sequences:
            ...
            elif vocab_flag==1:  # au
                seq_ = seq[:len(seq)] + [self.vocab_key['PAD']] * max(max_len - len(seq), 0)  # 截断和填充
            else:
                seq_=seq[:len(seq)] + [self.vocab_key['PAD']] * max(max_len - len(seq), 0)
    ...
    def train_and_eva(self, log_file=None):
        ...
        with tf.Session() as sess:
            ...
            ckpt_file = tf.train.latest_checkpoint('model_save_org/'.__add__('/task/'))
            ...
            for epoch_index in range(0, self.epoch_num, 1):
                ...
                if epoch_index % 200 == 0 and epoch_index != 0:
                    ...
                    print('time {}, step {}, train_loss: {:.4}, train_acc: {:.4}'.
                          format(start_time, epoch_index, loss_train, acc_))
    ...
    def predict_embedding(self):
        ...
        with tf.Session() as sess:
            ...
            for epoch_index in range(0, self.epoch_num, 1):
                start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            # Test Stage
                sentence_all, task_all = self.data_inter.next_all()  # 迭代器，每次取出一个batch块.
                if "array" not in str(type(sentence_all)):
                    print(np.array(label_list).shape)
                    pickle.dump(self.data_inter.end_all, open("Source/count_all.pkl", "wb"))
                    pickle.dump(label_list, open("Source/label.pkl", "wb"))
                    pickle.dump(feature, open("Source/org.pkl", "wb"))
                    print("predit done")
                    print('feature\'s shape{}'.
                          format(np.array(feature).shape))
                    break
                feed_dict_all, _ = self.get_feed_dict(sentence_all, task_all)
                feature1 = sess.run([
                    self.feature],
                    feed_dict=feed_dict_all)
                feature1=np.squeeze(feature1,axis=0)
                if epoch_index==0:
                    feature=feature1
                else:
                    feature = np.concatenate((feature, np.array(feature1)))
                label_list.extend(list(task_all[:, 1]))
                # print('feature\'s shape{}'.
                #       format(np.array(feature).shape))
    ...
    def get_feed_dict(self, seqs, labels=None, lr=None, dropout=None, predicted=False):
        ...
        """
        word_ids, seq_len_list = self.pad_sequences(seqs, pad_mark=0, predict=predicted, vocab_flag=2)#1
