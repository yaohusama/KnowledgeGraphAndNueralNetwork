...
class Data_Inter:
    ...
    def __init__(self, vocab_au, vocab_key):
        ...
        self.au_all, self.ke_all,self.org_all = self.get_author_art()
        print("self.au_all",self.au_all.shape)
        # print(self.au_all.shape, self.ke_all.shape,self.org_all)
        # TRAIN
    ...
    def get_data_mapping(self):
        ...
        self.org = self.org_all[self.shuffle_all[: int(self.all_sam * 0.7)]]
        ...
        self.org_test = self.org_all[self.shuffle_all[int(self.all_sam * 0.7):]]
        ...
        self.end_all=self.au_all.shape[0]
    ...
    def get_author_art(self):
        ...
        return np.array(auth), np.array(keys),np.array(org)
    ...
    def next(self):
        ...
        if self.index + self.config.batch_size < self.end:  # 没有遍历到末尾
            ...
            it_data = self.shuffle[self.index : self.end]#+ self.config.batch_size
        ...
        sentences_key = self.org[it_data]#ke
    ...
    def next_test(self):
        ...
        if self.index_test + self.config.batch_size <= self.end_test:  # 没有遍历到末尾
            ...
            it_data = self.shuffle_test[self.index_test: self.end_test]# + self.config.batch_size
        ...
        sentences_key = self.org_test[it_data]#ke
        for cur_sentences, cur_task in zip(sentences_au, sentences_key):
            task_.append(self.sentence2index(cur_sentences, self.vocab_au))  # author mapping
            sentence.append(self.sentence2index(cur_task, self.vocab_key))  # keys mapping
        return np.array(sentence), np.array(task_)
    def next_all(self):
        ...
        if self.index_all==-1:
            return -1,0
        if self.index_all + self.config.batch_size <= self.end_all:  # 没有遍历到末尾
            # 从task_sentence里面获取self.config.batch_size大小的块
            it_data=[i for i in range(self.index_all,self.index_all+self.config.batch_size)]
            self.index_all += self.config.batch_size  # 标识获取批次的索引值随批改变
        elif self.index_test + self.config.batch_size == self.end_all:  # 刚好遍历到末尾
            it_data=[i for i in range(self.index_all,self.index_all+self.config.batch_size)]
            # self.shuffle_test = sample(range(0, self.end_test, 1), self.end_test)
            self.index_all = -1  # 重置index。非常简单的小学数学问题，后面不在注释了，一眼便懂
        else:
            it_data=[i for i in range(self.index_all,self.end_all)]
            # self.shuffle_test = sample([0], self.end_test)
            remain =[0 for i in range(self.index_all + self.config.batch_size - self.end_all)]  # 剩余
            it_data = np.concatenate((it_data, remain), axis=0)
            self.index_all = -1

        sentences_au = self.au_all[it_data]
        sentences_key = self.org_all[it_data]#ke
        for cur_sentences, cur_task in zip(sentences_au, sentences_key):
            task_.append(self.sentence2index(cur_sentences, self.vocab_au))  # author mapping
            sentence.append(self.sentence2index(cur_task, self.vocab_key))  # keys mapping
        return np.array(sentence), np.array(task_)
