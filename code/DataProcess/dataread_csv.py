...
def get_author_article_mapping(pkl_path='all.pkl'):
    ...
    for cur_data in data:
        print(cur_data)
        # cur_data=cur_data1.split(",")
        author, article,org = cur_data[3:]
        ...
        org_pair=[]
        if len(author) == 1:
            author_pair = [[author[0], 'A_NA', article,org]]
        else:
            for j in range(1, len(author), 1):
                author_pair.append([author[j - 1], author[j], article,org])
        # print(author_pair)
        # print(article)
        data_save.append(author_pair)
        # print(' '.join(author) + '\t' + ' '.join(article)+"\t"+" ".join(org))
        # print('*' * 100)
    pkl.dump(data_save, open('data_manage.pkl', mode='wb'))
get_author_article_mapping()
...
def article_mapping():
    ...
    for j in data:
        for k in j:
            if len(k[-2]) > max_key:
                max_key = len(k[-2])
    print('max_key:', max_key1)
    # mapping
    for index_data in range(len(data)):
        for index_index_data in range(len(data[index_data])):
            # print(data[index_data][index_index_data][-2])
            if len(data[index_data][index_index_data][-2]) < (max_key * .6):
                data[index_data][index_index_data][-2] = data[index_data][index_index_data][-2] + \
                                                    (int(max_key * .6) - len(data[index_data][index_index_data][-2])) * ["PAD"]
            # print(data[index_data][index_index_data][-2])
            # print('*' * 100)
    for index_data in range(len(data)):
        for index_index_data in range(len(data[index_data])):
            # print(data[index_data][index_index_data][-1])
            if len(data[index_data][index_index_data][-1]) < int(max_key1 * .6):
                data[index_data][index_index_data][-1] = data[index_data][index_index_data][-1] + \
                                                    (int(max_key1 * .6) - len(data[index_data][index_index_data][-1])) * ["PAD"]
            # print(data[index_data][index_index_data][-1])
            # print('*' * 100)
